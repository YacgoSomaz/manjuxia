...
test("Electron scans known backend ports when backend.port is stale", () => {
  ...
  assert.match(source, /for \(const port of backendPortCandidates\(\)\)/);
  assert.match(source, /const candidateUrl = `http:\/\/127\.0\.0\.1:\$\{port\}`/);
});
