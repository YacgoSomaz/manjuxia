...
- 发布状态：`0.1.28` 商业安装包已构建，发布前测试、后端冒烟测试和发布扫描通过
- 当前版本：`0.1.28`
- 安装包：`packaging/release/installer/comic-shrimp/0.1.28/漫剧虾Setup_0.1.28.exe`
- 安装包 SHA-256：`a53d92eb0ad67cfeb13ea6c29262eedc33f465e8aa34421a4000327d013b6d16`
- 安装包大小：`244051356` 字节
- 代码签名：当前为 `NotSigned`；正式对外发布前需使用证书重新构建
- 版本号：由 `packaging/build/Publish-ComicShrimp.ps1` 根据已有发布目录递增；不要只改 `package.json`

分镜模板当前使用千山清洗版 30 条正式模板：ID23–ID51、ID62。正文随本地模板种子进入构建，并在本地 SQLite 中按 `qianshan_id` 复刻选择器顺序；旧版、测试版和差异备份不进入正式选择器。模型配置仍然是用户在漫剧虾本地设置的配置，不依赖千山远端模板接口。
...
完整性校验采用签名的 `version=2 / scope=core` 清单：只校验启动、鉴权、本地桥接、更新逻辑和编译后端主程序，不扫描清单外的用户图片、视频、音频、数据库、缓存、Python 运行库或 DLL。用户运行过程中新增文件不会再导致启动失败；核心文件哈希或清单签名被篡改仍会拒绝启动。这个规则需要重新构建安装包后才会对用户生效。

商业构建会先混淆所有自有 JavaScript，再将 Electron 主进程模块编译为与当前 Electron 运行时匹配的 V8 `.jsc` 字节码，并打入 `app.asar`。安装包内只保留极小主进程加载器；preload 与渲染页脚本因 Electron 隔离渲染上下文限制保留混淆脚本。账号权益的最终裁决仍由本地 Nuitka 后端验证 anyq.site 签发的 Ed25519 `account_license`，客户端字节码不能替代服务端校验。
...
千山近期功能差异已按本地优先边界补齐：信息提取页增加音色列表、试听和本地音频绑定；分镜页提供补镜视频任务入口；视频页提供小云雀 CLI 配置/检查和统一轮询；俯视人物调度图可以作为视频首尾连续性素材使用。小云雀凭证只保存到本机，语音试听资源随客户端发布，不读取千山登录 Cookie。

信息提取页还提供千山原版“人物描述润色”入口，可用当前本地语言模型配置返回润色后的人物描述，用户确认后再保存；分镜/视频页提供“恢复链路”，用于串行尾帧失败后恢复后续已中断分镜。两者都只操作本机数据库和本地模型配置，不调用千山云端模型配置。
...
实时更新产品 ID 写死在客户端：漫剧虾只能使用 `comic_shrimp`；运营虾客户端应单独固定为 `operation_shrimp`，不能由网页、配置文件或用户输入切换。旧安装包没有 SSE 实时监听能力，必须重新打包并安装新版本后才会生效。普通更新只在工作台左下角显示非打扰通知，用户主动点击后才下载；签名载荷中的 `mandatory=true` 或当前版本低于 `min_supported_version` 时才会弹出不可关闭提示并阻断使用。下载、签名、大小或 SHA-256 失败会显示错误并允许重试。
...
现有语言和图片模型下拉菜单提供两个彼此独立的 AI 来源：

- **本地自定义配置**：沿用原有本地加密模型配置，API 地址、Key 和模型名只保存在用户本机；选择该来源时不经过官方算力接口。
- **官方 AI 算力**：固定使用 `comic_shrimp` / `comic_course`，主进程只向 `https://anyq.site` 的官方目录、任务创建和任务查询接口发起请求。只有目录同时返回 `enabled=true`、`available=true` 的任务才会出现在下拉菜单中。客户端不保存或发送模型地址、API Key、模型名、provider 或 system prompt，价格、余额、是否开放和扣费由服务端决定。

官方算力未配置时显示“官方算力暂未开放”，不会偷偷切换到本地模型；余额不足、会员未开通、限流和上游异常分别提示用户。任务重试复用同一幂等键，服务端负责预扣、退款、幂等和最终权益校验。官方请求只在主进程携带登录会话，渲染层不会接触 Cookie。图片结果优先展示 `result_assets[].display_url`，缺失时兼容 `download_url`，保存动作由主进程完成；官方语言任务完成后保存回本地剧本表。

模型选择旁会显示“自配算力 / 官方算力”来源按钮。当前官方目录开放的语言和图片任务可直接从原有下拉菜单选择；视频、语音若服务端尚未开放，会明确显示“暂未开放”，不会伪造模型或偷偷切换来源。账号页脚从已验签的 `account_license.payload` 显示“积分余额”；服务端未返回签名总余额时显示“-”。未登录时显示“登录”按钮，只有已存在账号会显示“退出登录”。工作台侧边栏新增“历史成片”，只读取本机已完成并落库的视频记录。
