...
- 人物音频、人物马甲音频和“我的音色”导入现在会将 `.m4a` 自动转换为高质量 `.mp3`，数据库及视频生成链路使用 MP3；原始 M4A 文件会保留在本机素材目录。
- 视频提交前新增统一兜底：历史项目中已保存的 M4A 首次用于视频生成时也会自动补转为 MP3，因此火山、NewAPI、MiniMax 等不接受 M4A 的视频模型不会收到该格式。
- 转码使用安装包内置 FFmpeg；找不到组件、源文件无音轨、转码失败或超时会给出明确错误，不会把 M4A 静默提交给上游。
- 已构建 Windows 安装包 `漫剧虾-0.1.42-Setup.exe`，大小 `230338464` 字节，SHA-256 为 `390016d8e235bfe96446bb20a5d004c7798467aec77f37c4ba521ffa8605525e`；当前未进行 Authenticode 代码签名。

## 0.1.41 - 2026-09-17

### MiniMax H3 分辨率提交

- 修复 MiniMax H3 的提交路径在调用供应商前又将分辨率覆盖为 `2K` 的问题；现已实际验证 `768P` 和 `2K` 会分别进入最终上游请求参数。
- Windows 安装包不启用开发验收拦截，也不会向用户弹出调试日志；运行记录只写入用户本地 `app.log`，用于排障。
- 已构建 Windows 安装包 `漫剧虾-0.1.41-Setup.exe`，大小 `230335940` 字节，SHA-256 为 `c2e465100bdd4fdc59538421be295813d51a421d049682d797b49fb8e877c8fc`；当前未进行 Authenticode 代码签名。

## 0.1.40 - 2026-09-16

### NewAPI 视频素材路径修复

- 修复 NewAPI/官方视频在准备本地素材时只识别 `/data/...`、遗漏历史 `data/...` 路径格式的问题。后一格式不再被错误解析到安装目录，角色、场景和道具图会从用户数据/媒体目录读取并上传。
- 该修复覆盖“图片已在信息提取页显示、视频生成却报找不到 image 素材文件”的场景，尤其是卸载重装或后端工作目录变化后。

### MiniMax H3 分辨率

- MiniMax H3 视频页新增可选的 `768P` 与 `2K` 分辨率；选择会保存在本机，下次打开仍保留。
- 视频提交不再硬编码 `2K`，后端会校验并将用户选择的 `768P` 或 `2K` 原样传给 MiniMax V2 视频生成接口。
- 修复提交路由在调用 Provider 前再次把分辨率覆盖回 `2K` 的问题；现在前端选择、服务端记录和实际的上游请求保持同一分辨率。

### 构建产物

- 已构建 Windows 安装包 `漫剧虾-0.1.40-Setup.exe`，其中包含 NewAPI 视频素材路径修复和现有充值体验改动。
- 安装包大小为 `230334828` 字节，SHA-256 为 `9737688208660fd913b8596b8d2a9250a34a480210769f78138420d160eef31e`。
- 当前安装包未进行 Authenticode 代码签名。

## 0.1.39 - 2026-09-13

### 积分充值体验

- 客户端内“快速充值”在微信支付成功后会立即重新拉取账户余额，并以醒目的成功状态显示“充值成功”和实际到账积分。
- 成功提示会保留约 2.2 秒供用户确认，随后自动关闭充值弹窗；无需手动关闭，也不会再遗留过期的支付二维码。
- 保留应用重新获得焦点时的账户刷新，兼容用户仍通过浏览器完成支付的场景。

### 构建产物

- 已重新组装 Windows 安装包 `漫剧虾-0.1.39-Setup.exe`，其中包含当前前端充值逻辑、受保护 Electron 主进程与完整本地运行时。
- 安装包大小为 `230335450` 字节，SHA-256 为 `9b041453172a044a3938c9218cc26bf50be3f3b35f05483df6dc296210448197`。
- 当前安装包未进行 Authenticode 代码签名。

## Unreleased - 漫剧虾积分充值（2026-09-12）

- 左下角账户卡片的“积分余额”右侧新增“充值”按钮。桌面端通过现有一次性网页登录交接票据打开充值页，不把长期登录 Cookie 或原始 token 放进 URL。
- 新增固定积分档位：1 积分 ¥0.10（测试档）、500 积分 ¥50、2000 积分 ¥200、10000 积分 ¥1000；兑换比例固定为 ¥0.10/积分。
- 微信支付下单与已存在的验签回调共用同一订单表，但积分订单标记为 `credit_topup`。回调仅在验签、商户、订单金额都匹配且状态为成功时入账，并写入独立积分流水；不会创建或续期会员产品记录。
- 充值页在支付完成后轮询订单状态并提示到账；客户端刷新账户信息后显示服务端余额。
- 官网产品购买页在三张产品卡片下新增“漫剧虾算力积分充值”区；客户端登录交接后会以站内会话加载该区，不在 URL 传递原始 token。
- 已上线 `GET /api/pay/credit-plans` 与积分订单结算分支；未登录访问返回 401，登录后返回固定档位。原有会员支付、订单和权益续期流程保持不变。
- 客户端从浏览器支付返回并重新获得窗口焦点时，会主动刷新账户和官方算力余额，避免账户卡片继续显示支付前的缓存积分。
- “充值”按钮改为客户端内快速充值弹窗：档位由服务端读取，支付二维码与到账轮询留在客户端内完成；浏览器仍只用于会员产品购买。

## Unreleased - 官方视频算力与配置入口检查点（2026-09-12）

- 视频模型选择器补充显示型官方配置：火山方舟 `官方 Seedance2`、NewAPI 中转 `官方 NewAPI · MiniMax H3（768P）`，以及原生 `官方 MiniMax H3`。这些条目只传递内部配置 ID；客户端不保存或展示服务端 API Key。
- 将官方视频任务的服务端协议实现一并纳入项目：包括任务参数校验、NewAPI / 火山方舟 / MiniMax H3 提交与轮询、长任务恢复、超时处理，以及仅限必要字段的失败诊断。
- 官方图片任务继续在服务端完成上游调用与结果落库；提交源码中不包含真实服务密钥。测试桩使用的虚构密钥字符串仅用于验证脱敏边界。
- 本次为源码同步检查点，未执行任何付费图片或视频模型调用，未构建或发布安装包。开发目录存在与同步仓库不一致的其他历史工作文件；因会话上下文不完整，未擅自合并这些未审计差异。
